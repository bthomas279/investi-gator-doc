/* ==========================================================================
   ...
   the page. Every field except `title`, `date`, and `type` is optional.
     ...
     type      "major" — big card with a thumbnail (see `thumb`)
               "minor" — compact one-line row with a wrench icon
...
var UPDATES = [
     ...
     there is a real one to take its place. */
  {
    ...
    sections: [
      {
        text:
          "The data ranged from in-distribution testing (in_dist_test), datasets from the authors of part of the data (ood_mage_gpt, ood_mage_gpt_para), " +
          "data from more recent models (ood_gsingh), and cut-out domains/models not included in training (ood_raid_llama, ood_raid_reviews). The results of the tests can be seen below.",
      },
      {
        media: {
          kind: 'image',
      ...
      {
        text:
          "As you can maybe tell, it didn't perform too well. Turns out the LLM was extremely overconfident. To make a long story short, it naturally had a much higher confidence threshold so the line between a AI and human dection " +
          "was very thin and high (ex: 0.998...). That basically caused a lot of human posts to be labels as AI since the threshold line was much lower (0.5). While I can't figure out exactly why the model is so overconfident, it's likely due to the " +
      ...
      },
      {
        media: {
          ...
          caption: 'New OOD metrics with a margin threshold of 6.36. While the false negative ratio increased, I believe the new fpr is more than worth it.',
          // The image is a wide 1331x208 strip, so it gets its own frame
          // shape instead of being cropped into the default 16:9 one.
          ratio: '1331 / 208',
          fit: 'contain',
      ...
      { 
        text:
          "I implemented margin into the extension for AI text specifically. It stopped a lot of the false flags for posts but it came at a cost:",
...

  {
    ...
    title: 'Investi-gator Public Overview and first demo video',
    date: '2026-07-21',
    thumb: {
    ...
    excerpt:
      "Investi-gator's public overview page is live, along with a walkthrough video of the " +
      'extension badging posts as they load.',
    sections: [
      {
        heading: 'Added',
        items: [
